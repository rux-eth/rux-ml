...
| [PR-027](../../prs/PR-027-int64-timestamp-time-unit.md) | **Tier-2** Int64-timestamp time-unit fix on `TimeSeriesSplitter._effective_gap`. Empirical bug: `.cast(pl.Datetime("ns"))` on a `pl.Int64` Unix-seconds column reinterprets integers as ns (no unit conversion), driving `_effective_gap` to ~1e11 rows and crashing sklearn `TimeSeriesSplit`. Surfaced during PR-025 Phase 2; sidestepped via `PanelCombinatorialPurgedCV` for crypto-h3 at the time. PATCH; rolls into v0.2.0 at PR-026 | `[x]` | — |
| [PR-026](../../prs/PR-026-v0-2-0-cut.md) | **Tier-2** v0.2.0 version cut — script versioned→versioned migration (`docs/0.1/* → docs/0.2/*` in CLAUDE.md / README.md / `docs/SSOT/*`); bump `pyproject.toml` version `0.1.0 → 0.2.0`; rewrite `CHANGELOG.md` `[Unreleased]` → `[0.1.1]` + `[0.2.0]`; tag `v0.1.1` retroactively at `027c3b0` + `v0.2.0` at this merge commit. Analog of PR-021 for the v0.1 cut | `[x]` | PR-023, PR-024, PR-025, PR-027 |
