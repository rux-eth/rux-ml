...
| [PR-026](../../prs/PR-026-v0-2-0-cut.md) | v0.2.0 version cut | `design-research ~` — pattern locked by PR-021 (the v0.1.0 cut analog). | (1) Verify `scripts/rewrite_doc_refs.py` PATH_REWRITES + OPT_OUT_GLOBS still work post-PR-023's docs changes. (2) Phase 3 of the eval_set / PR-022 sentinel pattern (already cited; light state assessment only). (3) Roll PR-022's `[Unreleased]` entries into a `[0.1.1]` section ALONGSIDE the `[0.2.0]` section, OR cut v0.1.1 separately before v0.2.0 (decide at cut time). (4) Amend the CHANGELOG `[Unreleased] ### Changed` entry for PR-025 to match the committed calibration receipt numerics (B median = 0.027108 not 0.027101; C vs A = +0.42% not +0.26%; add B wallclock = −29% vs A). |
