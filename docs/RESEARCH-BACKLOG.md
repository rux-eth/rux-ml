...
| [PR-007](../prs/PR-007-optuna-basics.md) | Optuna basics — reclassified Tier-2 (2026-05-16) | `design-research ✓` (D6, D16) → reclassified Tier-2 → **`fully-researched 2026-05-16`** + **`implementation-cleared 2026-05-16`**. Resolved: (1) canonical objective shape = **K-fold CV-mean** (convention; NVIDIA Grandmasters + Optuna `OptunaSearchCV`); arithmetic mean aggregation; (2) **WilcoxonPruner at fold granularity** for the CV regime — **do NOT use `XGBoostPruningCallback` inside CV** (Optuna #3203); XGBoost-internal `early_stopping_rounds` per fold instead; (3) **TPESampler default** with `constant_liar=True` (AutoSampler convention); GPSampler for purely-numerical sub-studies up to 250 trials (AutoSampler `_MAX_BUDGET_FOR_SINGLE_GP`); HEBO opt-in via `optunahub`; (4) `BoTorchSampler` **rejected** — deprecated in Optuna 3.6, ~5× slower than GPSampler with no cited tabular-GBM advantage. `TuningConfig` schema narrowed (`sampler ∈ tpe/gp/hebo`; `pruner` default → `wilcoxon`; `constant_liar` field added). |
