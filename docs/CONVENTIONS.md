...
Hash elision: paths, timestamps, and runtime-only fields (`logs.path`, `studies.storage_url`) are excluded from `*_cfg_hash` computation. So is output-neutral guard config (per PR-040): a field whose only effect is to refuse a run can be elided, because any run that proceeds computed exactly what it would have without it. `data.oracle` is the only such field. A new Optional config field that is **not** elided still moves `*_cfg_hash` for every config, even while it is unset, because the dump gains `"<field>": null` (PR-040 Group D). The elision list lives as a constant `_HASH_ELIDED_FIELDS` in `src/rux_ml/config/root.py`.
