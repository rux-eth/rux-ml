...
| **MINOR** (`0.x → 0.x+1`) | New user-visible CLI verb or flag; new layer factory (`make_*`); new built-in `Trainer` or `Splitter` family; new study/problem-level config knob; new `manifest.json` field; new `user_attrs` field. | **Yes** — see §2. |
