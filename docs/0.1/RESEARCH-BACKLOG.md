...
| [PR-021](../../prs/PR-021-v0-1-0-cut.md) | v0.1.0 version cut | `design-research ~` → **`fully-researched 2026-05-18`** + **`implementation-cleared 2026-05-18`**. Resolved: (1) Q-A6 — path-glob opt-out (`prs/`, `docs/0.0/`, `docs/0.1/`, `src/`, `tests/`) + inline `<!-- rewrite-doc-refs:skip-line -->` sentinel for historical citations in mixed files (CLAUDE.md, README.md, docs/SSOT/*). Phase 3 surveyed scikit-learn / pandas / mike / SQLAlchemy / Sphinx — none publish tooling for the "active pointer vs frozen citation" distinction; ecosystem consensus is per-version doc-site rebuild + manual discipline. Sentinel comments are the closest documented practice (intersphinx version-pin analog). Idempotency verified: re-run of `make rewrite-doc-refs-dry` after apply reports 0 changes. (2) CHANGELOG audit — `[Unreleased]` already consolidated entries from PR-016 / PR-017 / PR-018 / PR-019 / PR-020; renamed to `[0.1.0] - 2026-05-18` + opened fresh `[Unreleased]` + added `[0.1.0]` compare link. (3) Post-merge tag command — annotated unsigned (single-user workbench): `git tag -a v0.1.0 -m "v0.1.0 — multi-family Trainer/Solver workbench" && git push origin v0.1.0`. Verification: `git describe --tags` matches `v0.1.0`; `uv run python -c "import rux_ml; print(rux_ml.__version__)"` returns `0.1.0`. (4) `Landed-in:` backfill — flipped PR-017 (`(not yet landed)` → `v0.1.0`) + PR-018/019/020 (`v0.1.0 (pending v0.1.0 cut in PR-021)` → `v0.1.0`); PR-021 stays at `(not yet landed)` per self-reference limitation (memory `feedback_pr_spec_historicity`). |
