...
| [PR-020](../../prs/PR-020-solver-protocol.md) | **Tier-2** Solver Protocol + first Solver family (bundled to avoid phantom) — new `Solver` `typing.Protocol` + `SOLVER_FAMILIES` registry + `make_solver` dispatcher + `SolverResult` dataclass. **First family: CVXPY** (zero dep addition — cvxpy + clarabel already transitive via skfolio; promoted to direct deps). Q-Shape **B-partial-mirror**: reuses config / registry / Optuna substrate, bypasses `cfg.data` / `cfg.cv` / fittable-model bundle. `rux-ml solve` single command (analog of `train`); solver-internal HPO deferred to follow-up. `TrialAttrs` extended with Optional solver fields. | `[x]` | PR-017 |
| [PR-021](../../prs/PR-021-v0-1-0-cut.md) | **Tier-2** v0.1.0 version cut — update `scripts/rewrite_doc_refs.py` PATH_REWRITES for `docs/0.0/* → docs/0.1/*` migration; resolve the versioned→versioned mapping wrinkle (A6 — opt-out list or smarter regex for historical citations); bump `pyproject.toml` `version` `0.0.1 → 0.1.0`; bump `src/rux_ml/__init__.py` `__version__`; rewrite `CHANGELOG.md` `[Unreleased]` → `[0.1.0] - YYYY-MM-DD`; tag `v0.1.0` at merge per `docs/VERSIONING.md §7` | `[x]` | PR-017, PR-018, PR-019, PR-020 |
