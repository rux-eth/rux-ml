...
| [PR-031](../../prs/PR-031-hpo-honors-holdout-fold.md) | **Tier-2** HPO objective honors holdout fold — `tuning/objective.py` builds CV substrate from `make_splits(...)["train"] + ["val"]`, not `df_full`. Test fold becomes truly held out from HP search. Behavior-changing: existing study metrics will shift. | `[x]` | PR-030 |
| [PR-032](../../prs/PR-032-registry-score-cli-verb.md) | **Tier-2** `rux-ml registry score` CLI verb + scorer module — loads bundle via `load_bundle`, reconstructs splits, scores `splits["test"]`, writes receipt JSON + preds parquet under `receipts/`. Removes stale "future golden-regression evaluation (PR-014)" comment from `promote.py:97`. | `[x]` | PR-031 |
| [PR-033](../../prs/PR-033-extmem-path-activation.md) | **Tier-2** ExtMem path activation — new `XGBoostNativeAdapter` (`training/xgboost/native_adapter.py`) wraps `xgb.train` in the `Trainer` Protocol with `select_ingest`-driven DMatrix construction; new `single_source_iter` helper chunks single-source Polars frames into N temp Parquets per the XGBoost 3.2 `external_memory.py` pattern; `cache_host_ratio` threaded through (GPU-only); `cfg.training.use_native` is now read in `cli/train.py`. **Scope reduced at Phase 1**: opt-in via `use_native=True` in baseline path only; `registry/promote.py` + HPO objective + `_check_extmem_compat` untouched. Covers phantom #2 + #2b. | `[x]` | PR-030 + design session |
| [PR-034](../../prs/PR-034-artifacts-store-integration.md) | **Tier-2** Optuna Artifacts Store integration — per-study `FileSystemArtifactStore(artifacts_root / study_name)` per Optuna 4.8 FAQ; in-objective post-fit upload of `metrics.json` (flat `Dict[str, float]` per MLflow + Kedro) + `fold_meta.json` (per-fold mixed-type sidecar); `cli/runs.py::show` lists `(filename, artifact_id)` pairs via `get_all_artifact_meta`. **Phase 1 scope reduction**: per-row preds CSV ruled out (3.31 GB/trial × 100 trials = 331 GB infeasible). `TrialAttrs` schema unchanged (Optuna auto-persists in `system_attrs`); `registry/promote.py` UNCHANGED (D8/A1 re-fit preserved). | `[x]` | PR-030 + design session |
| [PR-035](../../prs/PR-035-tune-retry-trial-test.md) | **Tier-1** `tune retry-trial` integration test — `test_tune_retry_trial_*` in `tests/cli/test_tune_subcommands.py`. Exercises enqueue → optimize → verify. No production code change. | `[x]` | — |
| [PR-036](../../prs/PR-036-v0-3-0-cut.md) | **Tier-2** v0.3.0 version cut — `scripts/rewrite_doc_refs.py` PATH_REWRITES bumped to `docs/0.2/* → docs/0.3/*` (+ `docs/0.3/**` added to OPT_OUT_GLOBS); 8 files / 18 replacements applied via rewriter; pyproject `0.2.0 → 0.3.0`; CHANGELOG `[Unreleased]` folded into `[0.3.0] - 2026-05-22` with canonical Keep-a-Changelog section consolidation (Added / Changed / Fixed); one stale historical citation in `[0.2.0]` release notes manually re-pinned with sentinel; tag `v0.3.0` lands at PR-036's merge commit. Analog of PR-021 / PR-026; Phases 2-4 abbreviated per `feedback_phase1_scope`. | `[x]` | PR-031, PR-032, PR-033, PR-034, PR-035 |
