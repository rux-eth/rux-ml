...
**HPO CV substrate is `splits["train"] + splits["val"]`, not the full dataframe** (per PR-031, v0.3.0). `tuning/objective.py:build_objective` calls `_carve_substrate(base_cfg, df_full, target_col)` once outside the per-trial closure — internally `make_splits(cfg, df, seed=study_bag.split_seed)` with `study_bag = make_seed_bag(master_entropy=base_cfg.tuning.entropy, trial_number=0)` (same `trial_number=0` sentinel as the sampler seed at `cli/tune.py:73`) — concatenates `splits["train"] + splits["val"]` as the CV substrate, leaving `splits["test"]` truly held out from HP search. Applies to both `data.split_kind == "random"` (deterministic per study via the study-level seed) and `time_ordered` (deterministic by definition per PR-024). The held-out test fold becomes consumable by `rux-ml registry score` once PR-032 lands. Convention citation: AutoGluon `tabular-essentials.html` at SHA `f8c428cbbef3bc319ff3f7710f5900e65637f4c4` (`fit(train_data) → evaluate(test_data)`); sklearn user guide §3.1; Optuna issue #2184.
...
**This eval_set placement is a research-acknowledged pragmatic deviation, not a textbook-clean pattern.** Per PR-022 Phase 3 research (2026-05-18), it matches the library-blessed default of `xgboost.cv()` / `lightgbm.cv()` / `catboost.cv()` but produces an optimism bias in CV-reported metrics that compounds across HPO trials. Workbench CV scores should be treated as point estimates for HP ranking, not as unbiased generalization estimates. **PR-025 calibrated A vs B vs C empirically** on the crypto-h3 panel-CPCV configuration (10 Optuna trials per position, same splits + seeds): A median RMSE 0.027101, B median 0.027101 (identical to A within search noise), C median 0.027172 (+0.26%, 60% slower). Position A's "optimism bias" did not materialize empirically on this dataset. The workbench stays on Position A; the bias warning still stands (the empirical zero is dataset-specific). Alternative patterns (Position B: inner val carved from train fold; Position C: no early stopping in CV with post-CV retrain) remain explicitly available; revisit per-problem per memory `project_cv_strategy_tier2`. See `docs/CONVENTIONS.md` "HPO objective shape" for cited sources + the PR-025 calibration numbers, and `prs/PR-025-calibration-results.json` for the raw measurement receipt.
...
| **Optuna artifacts** | `studies/artifacts/<study_name>/<uuid4>` | `optuna.artifacts.FileSystemArtifactStore` (per-study base path per Optuna 4.8 FAQ) | Per-trial diagnostic artifacts (active via PR-034): `metrics.json` (flat `Dict[str, float]` per MLflow + Kedro convention — per-fold scores + `metric_mean` + `metric_std` + `n_folds` + `peak_rss_mb`) + `fold_meta.json` (mixed-type per-fold sidecar — `fold_idx` / `row_count` / `fit_seconds` / `timestamp`). Optuna auto-persists each `ArtifactMeta` into `trial.system_attrs["artifacts:<uuid4>"]`; no `TrialAttrs.artifact_ids` field needed. Retention: manual `rm -rf studies/artifacts/<study>/` (Optuna FAQ refuses to officially support delete). Per-trial bundle uploads / plot PNGs / per-row prediction CSVs deferred per the D8/PR-010 A1 re-fit-at-promote contract + the PR-034 Phase 1 disk-space ceiling (3.3 GB/trial × 100 trials = 331 GB ruled out). |
| **Model registry** | `registry/<problem>/<version>/...` | filesystem | Promoted bundles + manifest + atomic `champion.json` |
| **Holdout score receipts** | `receipts/holdout_score_<problem>_<date>.json` + `receipts/holdout_preds_<problem>_<date>.parquet` | JSON (Pydantic-validated) + Parquet | Per-bundle holdout-fold scoring outputs (per PR-032). JSON tracked (single source of truth for the metric); `.parquet` gitignored (large, regeneratable). Receipt body shape (`metrics` dict + `artifacts` dict) follows MLflow `EvaluationResult` / Kedro `tracking.MetricsDataSet` convention; provenance wrapper (`bundle_version`, `manifest_git_sha`, `promoted_from`, `holdout`, `scored_at*`) is rux-ml-specific per AWS ML Lens BP03 for the no-tracker-server constraint. |
...
Optuna SQLite is the **single source of truth** for trial history. The registry is a curated, promoted slice — promotion is explicit, never implicit-by-being-the-best. Diagnostic artifacts are uploaded by Optuna to `studies/artifacts/<study_name>/<uuid4>` post-fit per PR-034; promotion is independent and copies the production model bundle into `registry/<problem>/<version>/` so the registry is independently loadable.
